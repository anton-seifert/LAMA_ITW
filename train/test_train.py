...
def train(env, config: Optional[dict] = None):
    ...
    vec_check_env = make_vec_env(env, n_envs=1,env_kwargs={"model_path":modelpath}, vec_env_cls=SubprocVecEnv)
    ...
    vec_train_env = make_vec_env(env,n_envs=8, env_kwargs={"model_path":modelpath},vec_env_cls=SubprocVecEnv)
    vec_train_env = VecMonitor(vec_train_env, filename=f"./monitor_logs/logs_train{timestamp}" )
    ...
    model.learn(total_timesteps=1_0_000,callback=eval_callback,progress_bar=True)
