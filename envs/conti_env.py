...
class RobotWorldEnv(gym.Env):
    ...
    def __init__(self, config: Optional[dict] = None, render_mode: Optional[str] = None):
        ...
        #TODO: maybe add second pair of sensors
        ...
        # """
        #(pos, vel, torque, action)*4 motors
        number_of_agent_observations = 4*self.model.nu
        ...
        self.observation_space = gym.spaces.Dict(
            {
                ...
                #TODO: maybe add tcp with strong noise or approximation
    ...
    def _get_obs(self):
        ...
        agent_obs = np.concatenate([self.data.actuator_length.flat[:], #motor pos, tendon_length would return measurements, that arent measureable in the realitily e.g. when there is slack in the tendon
                                    self.data.actuator_velocity.flat[:], #motor vel
                                    self.data.actuator_force.flat[:], #motor torque
                                    self.last_action.flat[:]]).astype(np.float32)
        
        sensors_obs = np.concatenate([self.data.sensordata]).astype(np.float32)
        #TODO: if tcp activate in obs
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        #TODO: das geht noch deutlich besser in dem man so antagonistische paare bildet
        if(self.options is None):
            for i in range(100):
                #radnom torques
                #print("lala2")
                random_action = self.np_random.uniform(0, 1, size=self.model.nu)*5
                self.data.ctrl[:] = random_action
                
                #let the robot reach stable pos
                for _ in range(50): 
                    mujoco.mj_step(self.model, self.data)
                    #print("lala3")

                    #break if already at rest
                    kinetic_energy = 0.5 * np.sum(self.data.qvel**2)
                    if kinetic_energy < 0.5:
                        #print("stable pos found")
                        break
                ...
                self.data.ctrl[:] = np.array([0,0,0,0])
                mujoco.mj_forward(self.model, self.data)
        else:
            print("starting from options")
            self.data.ctrl[:] = self.options["start_torque"]
    ...
    def calculate_floor_distance(self):
        ...
        tcp_pos = self.data.site_xpos[self.tcp_id][2]
        ...
        distances = np.array([np.abs(self.floor_level - tcp_pos)])
        return distances
    ...
    def calculate_target_for_sphere(self):
        ...
        radius = self.np_random.uniform(low=0.2, high= 0.55)
